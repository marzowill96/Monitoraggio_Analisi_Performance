...
#%% POSSIBILITA' DI SCEGLIERE MENO DI 3 FONDI
        ...
        calcolo_dict['volatilità']['IIS'].loc['IIS'] = np.std(calcolo_dict['Fondi Azionari']['CTV Totale'][:durata_months].pct_change(), ddof=1) * np.sqrt(12) #volatilità annualizzata
        calcolo_dict['volatilità']['PIC'].loc['IIS'] = np.std(calcolo_dict['PIC']['PIC'][:durata_months].pct_change(), ddof=1) * np.sqrt(12) #volatilità annualizzata
        ...
        for c in selected_funds:
            if ripartizione[c] == 0:
                ...
                calcolo_dict['volatilità']['IIS'].loc[c] = np.std(calcolo_dict['ctv fondi'][c][:durata_months].pct_change(), ddof=1) * np.sqrt(12) #volatilità annualizzata
                calcolo_dict['volatilità']['PIC'].loc[c] = np.std(calcolo_dict[c]['df']['QUOTA'][:durata_months].pct_change(), ddof=1) * np.sqrt(12) #volatilità annualizzata
